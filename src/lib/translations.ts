      ...
      projects: [
        {
          ...
          y: "2026",
          description: "<strong>SkinCanvas</strong> — AI-сервис для тех, кто собирается сделать татуировку: помогает придумать эскиз, примерить его на фото своего тела, доработать уже существующую работу и найти мастера.\n\n<strong>Задача.</strong> Решиться на татуировку сложно: непонятно, как рисунок будет смотреться на теле, как совместить его со старыми работами, что показать мастеру и к кому идти. Нужен был сервис, где всё это можно увидеть и решить заранее — без походов по салонам и долгих переписок.\n\n<strong>Что сделал.</strong> Собственный проект — полностью в одиночку: от интерфейса и нейросетевых сценариев до платежей, юридической части и продвижения.\n\n<strong>Для пользователя</strong> — десять сценариев на нейросетях: бесплатный AI-консультант, генерация эскизов и рукавов, эскиз по любой картинке, определение стиля по фото, примерка рисунка на фото тела, доработка, связка и перекрытие существующих татуировок, эскиз для мастера. Результаты сохраняются в личную галерею.\n\n<strong>Примерка.</strong> Место татуировки задаётся схемой на фото, а не словами в промпте: модель ставит работу туда, куда её поставил человек. Рукав на согнутой руке рисунок повторяет изгибом — он гнётся по той же дуге и в превью, и на сервере.\n\n<strong>Мастера.</strong> Раздел «Найди своего мастера»: студии по городам, работы и контакты. Сервис собирает сообщение мастеру — идея, место, размер, эскиз из галереи. Страницы мастеров открыты для поиска, и студии ставят ссылку на них у себя.\n\n<strong>Деньги.</strong> Внутренний счёт с ценой на каждой кнопке, приём платежей через Robokassa с зачислением по подписанному уведомлению, защита от двойных списаний и автоматический возврат, если нейросеть не справилась. После подтверждения почты начисляются пробные единицы — человек видит результат раньше, чем достаёт карту.\n\n<strong>Надёжность.</strong> Очередь задач для долгих генераций, которая переживает перезапуск сервера; проверка запроса до платного вызова модели; защита загрузок от подмены и «бомб» в картинках; резервные копии, которые проверяются разворачиванием. Больше 100 файлов автотестов, проверки на каждый push в CI.\n\n<strong>Поиск и закон.</strong> Около 50 публичных страниц — сценарии, стили, места на теле, статьи, мастера — отрисовываются заранее, чтобы их видели Яндекс и Google. Данные хранятся на сервере в России по 152-ФЗ; передача фото в модель за рубеж согласована с Роскомнадзором, документы и согласия оформлены.\n\n<strong>Результат.</strong> Запущен в продакшен за 3 месяца — работает и принимает оплату.\n\n<strong>Стек:</strong> React · TypeScript · Vite · TanStack Query · Tailwind · Express · PostgreSQL · Prisma · Zod · OpenAI · MinIO (S3) · sharp · Robokassa · Docker · Caddy · nginx · GitHub Actions",
      ...
      projects: [
        {
          ...
          y: "2026",
          description: "<strong>SkinCanvas</strong> — an AI service for people planning a tattoo: it helps come up with a design, try it on a photo of your own body, rework a tattoo you already have, and find an artist.\n\n<strong>The goal.</strong> Committing to a tattoo is hard: it's unclear how the design will look on the body, how to combine it with existing work, what to show the artist, and who to go to. The service lets you see and decide all of that in advance — without visiting studios or long back-and-forth chats.\n\n<strong>What I built.</strong> A personal project, built entirely solo: from the interface and AI scenarios to payments, the legal side, and marketing.\n\n<strong>For users</strong> — ten AI-powered scenarios: a free AI consultant, design and sleeve generation, a design from any image, style detection from a photo, try-on on a body photo, reworking, connecting and covering up existing tattoos, and a sketch for the artist. Results are saved to a personal gallery.\n\n<strong>Try-on.</strong> The placement is marked on the photo, not described in the prompt: the model puts the design exactly where the person placed it. On a bent arm, a sleeve follows the curve — it bends along the same arc both in the preview and on the server.\n\n<strong>Artists.</strong> A \"Find your artist\" section: studios by city, portfolios, and contacts. The service drafts a message to the artist — idea, placement, size, and a sketch from the gallery. Artist pages are open to search engines, and studios link to them from their own sites.\n\n<strong>Payments.</strong> An in-app balance with the price shown on every button, Robokassa payments credited via a signed notification, protection against double charges, and automatic refunds when the model fails. Trial credits after email verification — users see a result before reaching for their card.\n\n<strong>Reliability.</strong> A job queue for long generations that survives server restarts; request checks before any paid model call; upload protection against spoofed files and image \"bombs\"; backups verified by actually restoring them. 100+ automated test files, with checks on every push in CI.\n\n<strong>Search and compliance.</strong> About 50 public pages — scenarios, styles, body placements, articles, artists — are pre-rendered so Yandex and Google can index them. Data is stored on servers in Russia under local data protection law (152-FZ); cross-border transfer of photos to the model is registered with the regulator (Roskomnadzor), with legal documents and consents in place.\n\n<strong>Result.</strong> Launched in 3 months — live in production and accepting payments.\n\n<strong>Stack:</strong> React · TypeScript · Vite · TanStack Query · Tailwind · Express · PostgreSQL · Prisma · Zod · OpenAI · MinIO (S3) · sharp · Robokassa · Docker · Caddy · nginx · GitHub Actions",
          liveLink: "https://skincanvas.ru",
        },
        {
          n: "00",
